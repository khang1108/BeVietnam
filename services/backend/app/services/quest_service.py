...
from models.quest import UserProgress

class QuestService:
    def __init__(self, repo: QuestRepository):
        self.repo = repo

    async def start_quest(self, user_id: int, chain_id: int, first_task_id: int):
    ...
    async def complete_current_task(self, user_id: int, chain_id: int, current_order_index: int):
