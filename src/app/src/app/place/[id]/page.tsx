      ...
      <div className={styles.detailContent}>
        <div className={styles.detailMain}>
          ...
          <div className={styles.placeholderSection}>
            ...
            <div className={styles.placeholderDesc}>
              {locale === 'vi' ? 'Hình ảnh sẽ được tải từ API' : 'Images will be loaded from API'}
            </div>
        ...
        <div className={styles.detailSidebar}>
          <div className={styles.sidebarCard}>
            ...
            <div className={styles.sidebarItem}>📍 {locale === 'vi' ? 'Chùa Một Cột, Ba Đình, Hà Nội' : 'One Pillar Pagoda, Ba Dinh, Hanoi'}</div>
            <div className={styles.sidebarItem}>🕐 {locale === 'vi' ? '7:00 - 18:00' : '7:00 AM - 6:00 PM'}</div>
            <div className={styles.sidebarItem}>💰 {locale === 'vi' ? 'Miễn phí' : 'Free'}</div>
            <div className={styles.sidebarItem}>⭐ 4.8/5</div>
          </div>

          <div className={styles.sidebarCard}>
            <h3>{locale === 'vi' ? 'Hành động' : 'Actions'}</h3>
            <div className={styles.sidebarItem}>🗺️ {locale === 'vi' ? 'Chỉ đường' : 'Get Directions'}</div>
            <div className={styles.sidebarItem}>📤 {locale === 'vi' ? 'Chia sẻ' : 'Share'}</div>
