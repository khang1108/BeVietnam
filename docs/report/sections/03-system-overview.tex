...
Sau khi đã phân tích bài toán ở
chương trước, ta tiến hành trình bày góc nhìn tổng thể về hệ thống
...
nhiều nền tảng khác nhau. Mỗi khi họ muốn đi du lịch, người dùng sẽ phải đi tìm
nhiều nơi khác nhau như TikTok, Facebook dẫn tới rất mất thời gian và công sức. Hoặc họ cũng có thể tìm thấy tên địa điểm trên
...
ban đầu rất rời rạc và thiếu đi các thông tin về địa điểm đó đặc biệt văn hóa. Vì vậy, feature
...
để hoạt động. Để xây dựng được dữ liệu đầu vào, nhóm đã tiến hành triển khai một giải pháp
OCR để có thể trích xuất chữ từ những cuốn sách văn hóa của địa điểm mà nhóm nhắm đến, từ đó
sử dụng một mô hình ngôn ngữ lớn là \texttt{Qwen2.5-14B} để từ đó có thể trích xuất ra các thông
tin dưới dạng một file JSON. Ta có thể hiểu feature này như một
``lớp biên tập số'' của toàn hệ thống, nó đóng vai trò là lên kịch bản và nội dung cho từng địa điểm,
dựa trên các yêu cầu, bài toán cụ thể đã được đưa ra. Thay vì ném cho du khách một danh
...
nghiệm khám phá mạch lạc, thông qua các bài post, các widget mang đến các thông tin
thú vị. Khi người dùng mở ứng dụng, họ không chỉ thấy
...
hay storyline. Feature này chính là tầng chuẩn hóa dữ liệu đầu
vào cho toàn bộ sản phẩm, là nơi sản sinh toàn bộ tri thứ của dự án. Backend sẽ phải tổ chức POI theo schema thống
nhất, để tạo ra thực thể \texttt{Place} có thể được dùng lại cho
...
liệu giúp mọi feature còn lại có thể hoạt động ổn định và nhtạo ra
...
Đây là feature gần nhất với giá trị cốt lõi của sản
...
quả, mà còn hiểu được lập luận phía sau kết quả. Dynamic Recommendation Feed là nơi mô hình quyết
...
Hình~\ref{fig:bubble_maps} minh họa bubble map ở mức tổng quan và khi mở
chi tiết một địa điểm.

\begin{figure}[H]
\centering
\IfFileExists{images/bevietnam_map.png}
  {\includegraphics[width=0.49\textwidth]{images/bevietnam_map.png}}
  {\fbox{\parbox[c][3cm][c]{0.49\textwidth}{\centering\itshape\color{darkgray}images/bevietnam\_map.png}}}
\hfill
\IfFileExists{images/bevietnam_detail_map.png}
  {\includegraphics[width=0.49\textwidth]{images/bevietnam_detail_map.png}}
  {\fbox{\parbox[c][3cm][c]{0.49\textwidth}{\centering\itshape\color{darkgray}images/bevietnam\_detail\_map.png}}}
\caption{Realtime Bubble Map. Trái: các bong bóng POI quanh người dùng,
kích thước co giãn theo khoảng cách. Phải: mở chi tiết một địa điểm.}
\label{fig:bubble_maps}
\end{figure}
...
gần nhau. Điểm quan trọng nhất của feature này là tính
\textit{thời gian thực} và làm sao để ta có quét được toàn bộ địa điểm và các điều kiện ngoại cảnh. Ở bản
...
bổ sung nguồn Foursquare thời gian thực. Foursquare được giữ ở backend để có thể
gửi các tọa độ về thông qua danh sách POI đã được phân loại theo nhóm (ẩm thực, lưu trú, văn hóa,
...
ảnh minh chứng nhằm mở khóa nút kế tiếp. Hình~\ref{fig:storyline} minh họa
lộ trình Mission Path và màn hình chi tiết một nhiệm vụ.

\begin{figure}[H]
\centering
\IfFileExists{images/storyline.png}
  {\includegraphics[width=0.49\textwidth]{images/storyline.png}}
  {\fbox{\parbox[c][3cm][c]{0.49\textwidth}{\centering\itshape\color{darkgray}images/storyline.png}}}
\hfill
\IfFileExists{images/storyline_detail.png}
  {\includegraphics[width=0.49\textwidth]{images/storyline_detail.png}}
  {\fbox{\parbox[c][3cm][c]{0.49\textwidth}{\centering\itshape\color{darkgray}images/storyline\_detail.png}}}
\caption{Storyline Mission Path. Trái: lộ trình nhiệm vụ kiểu Duolingo, mở
khóa tuần tự. Phải: màn hình chi tiết một nhiệm vụ với nội dung và nút nộp ảnh.}
\label{fig:storyline}
\end{figure}

Feature này làm cho chuyến đi có cảm giác được hướng dẫn
...
có thể thực hiện tại địa điểm thật. Storyline Mission Path là một bài toán quản lý tiến
...
quyết định nút nhiệm vụ có được mở khóa hay không. Feature này làm cho việc hoàn thành nhiệm vụ có ý nghĩa
...
không chỉ với thao tác trên màn hình. Đây là nơi hệ thống nối state số với bằng chứng
thực địa thông qua một mô hình thị giác. Lúc đầu, phần này thực
...
Việt hoặc tiếng Anh. Feature này quyết định việc họ có thật sự hiểu được giá
...
địa lẫn quốc tế. Nó yêu cầu cả client lẫn lớp nội dung
phải phối hợp chặt chẽ với nhau. Client chịu trách nhiệm chuyển đổi nhãn hiển thị,
...
% ---- vLLM layer ----
\node[llm, below=32mm of verify] (vllm) {Qwen2.5-VL-7B\\(vLLM, GPU L40)};
...
Trước hết, hai client web và mobile không nói
...
SerpAPI) được lưu backend giúp cho đảm bảo bảo mật và an toàn cũng như là có khả năng
chuẩn hóa dữ liệu trước khi trả về. Sau cùng, lớp vLLM
nằm tách hẳn và chỉ có duy nhất \texttt{/verify-capture} chạm tới, ứng với phần trước rằng xác minh ảnh là tính năng duy nhất bắt
buộc dùng mô hình AI tự host tại thời gian thực.

\subsection{Innovation Highlights}
\contrib
Như vậy sau khi đã phân tích và làm rõ các vấn đề thì điểm mới của BeVietnam không nằm ở việc sử dụng AI như một bộ não
chung chung, mà nằm ở cách hệ thống kết hợp nhiều thành phần công nghệ và
nghiệp vụ để giải quyết đúng Pain Point của người đi du lịch. Nhóm không tập trung
làm quá hầm hố cho công nghệ mà sẽ tập trung làm sao có thể giải quyết dứt điểm
Pain Point của người dùng. Các đổi mới
